...
import { assignTripSchema } from "@/validators/trip.validator";
...
async function handler(req: NextRequest) {
  ...
  // Validate input
  const input = await validateBody(assignTripSchema)(req);

  // Assign trip (includes transaction and availability checks)
  ...
  if (wantsJson) {
    return NextResponse.json({
      ok: true,
      notice: `Trip assigned to ${driver.name} • ${vehicle.number}`,
    });
  }

  // Redirect for form submissions
  const url = new URL("/transport", req.url);
  url.searchParams.set("notice", `Trip assigned to ${driver.name} • ${vehicle.number}`);
